...
// TODO: Wrong, I am missing something important...
class Solution {
private:
	int V;
	vector<int> res;
...
public:
    vector<int> loudAndRich(vector<vector<int>>& richer, vector<int>& quiet) {
		...
		for (vector<int>& neighbors : adj){
			// sort by quietness (most quiet to least quiet)
			sort(neighbors.begin(), neighbors.end(), [&](int a, int b){ 
				return quiet[a] < quiet[b]; // Does not solve all test cases.
			});
		}
		...
		for (int leaf : leaves){
			set<int> vis;
			cout << "DFS into: " << leaf << "\n";
			dfs(leaf, vis, quiet, leaf);
			cout << "\n";
		}
		return res;
    }
private:
	void dfs(int node, set<int>& visited, vector<int> quiet, int most_quiet_node){ 

		visited.insert(node);
		if (quiet[node] < quiet[most_quiet_node])
			most_quiet_node = node;
		cout << node << "(" << most_quiet_node << ")->";
		if (res[node] == INT_MAX)
			res[node] = most_quiet_node;
		else if (quiet[res[node]] >= quiet[most_quiet_node])
			res[node] = most_quiet_node;

		for (int neighbor : adj[node])
			if (visited.find(neighbor) == visited.end())
				dfs(neighbor, visited, quiet, most_quiet_node); 
	}
